...
for i in range(len(text_test)):
    if y_test[i] != classifier.predict(X_test_norm[i]):
...
print("Accuracy on test data - method score: {:.2f}%".format(classifier.score(X_test_norm, y_test) * 100))
print('Accuracy on train data - method score: {:.2f}%'.format(classifier.score(X_train_norm, y_train) * 100))

scores = cross_val_score(classifier, X_train_norm, y_train, cv=5)
